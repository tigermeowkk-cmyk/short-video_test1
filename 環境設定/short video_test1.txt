...
def generate_saas_video(user_media_paths, style_id, slogan_text, shop_name, logo_path, output_path):
    ...
    if slogan_text:
        title_clip = TextClip(
            slogan_text, fontsize=60, color=cfg["text_color"], font=cfg["text_font"],
    ...
    # 音樂庫隨機抽選 (雲端若無對應資料夾則跳過)
    bgm_dir = f"./bgm/{style_id}"
    if os.path.exists(bgm_dir) and os.listdir(bgm_dir):
        ...
        bgm_audio = AudioFileClip(selected_bgm).subclip(0, video_duration).volumex(bgm_volume)
...
st.subheader("影音業專屬測試版 ── 一鍵生成你的爆款短影音")
